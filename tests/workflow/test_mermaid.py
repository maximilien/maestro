...
class TestWorkflow_to_mermaid(TestCase):
    def setUp(self):
        self.workflow_yaml = parse_yaml(
            os.path.join(
                os.path.dirname(__file__), "../yamls/workflows/simple_workflow.yaml"
            )
        )[0]
...
class TestMermaidCondition_case(TestCase):
    def setUp(self):
        self.workflow_yaml = parse_yaml(
            os.path.join(
                ...
                "../yamls/workflows/conditional_case_workflow.yaml",
            )
        )[0]
    ...
    def _test_markdown_flowchart(self):
        for orientation in ["TD", "LR"]:
            mermaid = Mermaid(self.workflow_yaml, "flowchart", f"{orientation}")
            self.assertTrue(
                mermaid.to_markdown().startswith(f"flowchart {orientation}")
            )

            self.assertTrue("agent1-- step1 -->agent2" in mermaid.to_markdown())
            self.assertTrue(
                "agent3-- step2 (input.some_condition == True) -->agent2"
                in mermaid.to_markdown()
            )
            self.assertTrue(
                "agent3-- step3 (input.some_condition == False) -->agent3"
                in mermaid.to_markdown()
...
class TestMermaidCondition_if(TestCase):
    def setUp(self):
        self.workflow_yaml = parse_yaml(
            os.path.join(
                os.path.dirname(__file__),
                "../yamls/workflows/conditional_if_workflow.yaml",
            )
        )[0]
...
class TestMermaid_event_cron(TestCase):
    def setUp(self):
        self.workflow_yaml = parse_yaml(
            os.path.join(
                os.path.dirname(__file__),
                "../yamls/workflows/simple_cron_workflow.yaml",
            )
        )[0]
...
class TestMermaid_parallel(TestCase):
    def setUp(self):
        self.workflow_yaml = parse_yaml(
            os.path.join(
                os.path.dirname(__file__), "../yamls/workflows/parallel_workflow.yaml"
            )
        )[0]
...
class TestMermaid_loop(TestCase):
    def setUp(self):
        self.workflow_yaml = parse_yaml(
            os.path.join(
                os.path.dirname(__file__), "../yamls/workflows/loop_workflow.yaml"
            )
        )[0]
