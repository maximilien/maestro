...
def deploy_meta_agents_streamlit(prompt_text_file):
    ...
    try:
        ...
        Console.error(
            f"Unable to read prompt text file: {prompt_text_file} error: {str(e)}"
        )

    # Set page configuration
    st.set_page_config(
        page_title="Maestro Meta-Agents Workflow", page_icon="🤖", layout="centered"
    )
    ...
    agents_tab, workflow_tab, generated_workflow_tab = st.tabs(
        ["Agents", "Workflow", "Generated"]
    )

    with agents_tab:
        st.header("Meta-agents 🤖 -> agents.yaml")
        ma_agents_workflow_ui = StreamlitWorkflowUI(
            pkg_path("agents/meta_agent/agents.yaml"),
            pkg_path("agents/meta_agent/workflow_agent.yaml"),
            prompt,
            "Maestro meta-agents agents workflow",
            generated_agent,
        )
    ...
    with workflow_tab:
        ...
        ma_workflow_workflow_ui = StreamlitWorkflowUI(
            pkg_path("agents/meta_agent/agents.yaml"),
            pkg_path("agents/meta_agent/workflow_workflow.yaml"),
            prompt,
            "Maestro meta-agents workflow",
            generated_workflow,
        )
    ...
    with generated_workflow_tab:
        ...
        if agent_file is not None and workflow_file is not None:
            ma_gent_workflow_workflow_ui = StreamlitWorkflowUI(
                agent_file.getvalue().decode("utf-8"),
                workflow_file.getvalue().decode("utf-8"),
                "",
                "Maestro meta-agents generated workflow",
            )
            ...
            ma_gent_workflow_workflow_ui = StreamlitWorkflowUI(
                agent_file, workflow_file, "", "Maestro meta-agents generated workflow"
            )
            ma_gent_workflow_workflow_ui.setup_ui()
